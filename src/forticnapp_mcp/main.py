...
def load_settings_or_fail() -> Settings:
    """Shared by both entrypoints (stdio's _serve() and http_server.run()) so a bad
    config produces the same one-line stderr message regardless of transport."""
    try:
