...
# missing file, and validate_spec_path()'s retry-against-project-root below covers that
# case (Docker's WORKDIR reliably matches the project layout, so plain "./lw.yaml" works
# there without ever needing the retry).
...
class Settings(BaseSettings):
    ...
    def validate_spec_path(self) -> Path:
        ...
        forticnapp_openapi_spec is a plain cwd-relative path (default "./lw.yaml", or
        whatever FORTICNAPP_OPENAPI_SPEC is set to) -- MCP clients (observed: Claude
        Desktop) don't reliably chdir into the `cwd` set in their server config before
        spawning the process, so if it doesn't resolve from the process's actual CWD,
        retry it anchored to the project root before giving up. A no-op for absolute
